...
![图12:电路2-0](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图12:电路2-0.png)


![图12:电路2-0.png](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图12:电路2-0.png)

这个「图12:电路2-0」表示了这样的一个计算：
...
![图13:电路2-1
被隐去的 $m_1$](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图13:电路2-1.png)
...
之后，我们来表示电路的「运算态」，我们可以将 $(x_1 + 2) \cdot x_2 = out$ 表示为 $z$ 的线性组合，便于验证和计算：
...
\end{array}
$$
那么电路的左输入向量 $U$ 对应的选择子向量为 $U =[2,1,0,0]$，其中：

- `2` 对应常数项 $2$
- `1` 对应 $x_1$

其次是电路的右输入向量 $V$ 对应的选择子向量 $V=[0,0,1,0]$， 其中：
...
$$
\begin{array}{l}
(𝑈⋅𝑧)=2⋅1+1⋅𝑥_1+0⋅𝑥_2+0⋅𝑜𝑢𝑡=2+𝑥_1 \\

(𝑉⋅𝑧)=0⋅1+0⋅𝑥_1+1⋅𝑥_2+0⋅𝑜𝑢𝑡=𝑥_2 \\

(𝑊⋅𝑧)=0⋅1+0⋅𝑥_1+0⋅𝑥_2+1⋅𝑜𝑢𝑡=𝑜𝑢𝑡
\end{array}
$$
这个内积等式化简之后正好可以得到：
...
![图14:电路3
从 $(x_1+x_2)\cdot (2\cdot x_3)=out$ ⇒$(x_1+x_2)\cdot(x_3+x_3)=out$](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图14:电路3.png)

上面说的是电路本身是一个乘法门或者可以转换成一个乘法门来处理的情况，可如果一个电路含有两个以上的乘法门，我们就不能用  $𝑈,𝑉,𝑊$ 三个向量之间的内积关系来表示运算，而需要构造「三个矩阵」的运算关系。
...
![图15:电路4.png](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图15:电路4.png)
...
$$
![计算过程2-1.png](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/计算过程2-1.png)

![计算过程2-2](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/计算过程2-2.png)

![计算过程2-3](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/计算过程2-3.png)
