...
![图：电路1](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图：电路1.png)


「图：电路1」由几部分构成，其中有三个算术门。
...
从「图：电路1」中，我们可以找到里面对应的数学关系，也就是**这个电路要成立必须要满足的约束关系**：

![图2-约束关系](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图2:约束关系.png)
...
![图3:矩阵 $W\in\mathbb{F}^{n\times 3}$](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图3.png)
...
第一个门 `#1`，如果 $x_3\cdot x_4 - x_6=0$ 满足 $q_L \circ w_a + q_R \circ w_b + q_M\circ(w_a\cdot w_b) + q_C -  q_O\circ w_c = 0$ 这个固定等式关系，那么它的左输入选择器 $q_L$=0，它的右输入选择器 $q_R$=0，它的乘法选择器 $q_M$=1，常数选择器 $q_C$=0，输出选择器 $q_O$=1。
...
![计算过程1-1.png](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/计算过程1-1.png)
...
![计算过程1-2.png](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/计算过程1-2.png)
...
![图4:矩阵 $Q\in\mathbb{F}^{n\times5}$](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图4.png)


重点来了！

有了约束矩阵 $Q$ 的表格和和接下来，我们就可以来验证，我们构建的两个矩阵的计算是否满足一开始的等式，即：
...
![图5:矩阵 $Q$、$W$ 代入固定等式](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图5.png)
...
![图6:化简过程](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图6:化简过程.png)


![图7:化简后](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图7:化简后.png)
...
![图8:电路比较](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图8:电路比较.png)
...
![图9](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图9.png)

为了防止「恶意的」 Prover 作恶，我们需要增加新的约束，强制要求右边电路图中 $x_5=x_7$ 和 $x_6=x_8$，见图10。这等价于我们要求 Prover 把同一个变量填入表格多个位置时，**必须填入相等的值**。

![图10：新的约束关系](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图10：新的约束关系.png)
...
- **概念补充**
  ...
  假设我们有一个初始的列表（或排列）$*w_1*,*w_2*,*w_3*,…,*wn$*，每个 $*w_i$* 是一个元素。

  在描述中， $j (w_i,j) $ 表示元素 $*w_i$* 被交换到了新的位置 $*j$*。

![图11:$\sigma$ 向量](ZKP-PLONK/images/PLONK算术化「1」/PLONK 算术化 686a05557f74481f87a259ff88ffa3d8/图11.png)
