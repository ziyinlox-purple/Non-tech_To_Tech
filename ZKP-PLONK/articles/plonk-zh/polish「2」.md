...
其中，在「表1」中，定义域是 $i\in \{1,2,3\}$，看「2」会更加清晰；而在「表3」，插值多项式 $w_a(X)$ 一旦构建完成，自变量 $X$ 的取值范围就可以不局限于原来的 $\{1,2,3\}$ 了，可以扩大到有限域  $\mathbb{F}$ 中。

我们通过已有的三个插值点构造了「表3」 的拉格朗日插值多项式，有了它以后，我们可以带入未知点进行插值点的数值运算，验证是否满足 $w_a(X)\cdot w_b(X)=w_c(X)$。如果从 $\mathbb{F}$ 中随机挑选的 $X$ 可以满足 $w_a(X)\cdot w_b(X)=w_c(X)$，如果 $\mathbb{F}$ 的取值范围足够大，那么可以极大概率地证明 Prover 没有作恶。这是如何保证的呢？就要引入下面说到的 Schwartz-Zippel 引理了。
...
可以观察上面的等式， $a_i\cdot b_i$ 和 $c_i$ 的项并不对应到 $X^i$ 的系数，比如 $a_1\cdot b_1$ 的系数出现在 $X^2$ 上，但同时 $X^2$ 项的系数组成还有 $a_0b_2$ 和 $a_2b_0$。而在 $c(X)$ 中 $c_1$ 是 $X^1$ 的系数。

> - [ ] 图片中更改第二行等式多项式中 $(a_0b_1+b_0a_1)X$ 的下标，更改 $C_4$ 为 $c_4$

<img src="/ZKP-PLONK/images/polish「2」/对应.png" width="70%" />


因此**我们需要另一种多项式编码方案，利用拉格朗日基(Lagrange Basis)**。如果我们要构造多项式 $a(X)$，令它在 $H=\{w_0, w_1, \ldots w_{N-1}\}$ 上取值，假设这组取值为 $\vec{a}$，表示多项式 $a(X)$ 在某个有限域 $H$ 上的一组取值，即
...
换个说法， $f(X)$ 可以被多项式 $z_H(X)=(X-w_0)(X-w_1)(X-w_2)\cdots(X-w_{N-1})$ 整除，并得到一个商多项式 $q(X)$，它表示 $f(X)$ 除以 $z_H(X)$ 后得到的结果。零化多项式 $z_H(X)$ 又被称为消失多项式(Vanishing Polynomial)，它捕捉了 $f(X)$ 的所有根。
...
> 如果是一个非零的多项式 $P(X)=a(X)\cdot b(X)-c(X)$，假设次数为 $d$，那么随机选择一个数代入验证，得到结果为零的概率 $\le \frac{d}{|\mathbb{F}|}$，其中 $\mathbb{F}$ 是有限域。前提是当 $\mathbb{F}$ 足够大的时候，得到结果为 0 的概率 $\le \frac{d}{|\mathbb{F}|}$， 可以忽略不计。所以随机找一个在 $\mathbb{F}$ 中的数代入，如果多项式 $P(X)=0$，那这个 $P(X)$ 大概率就是零多项式 $z_H(X)$。 

> ❓ 那这个 $P(X)$ 大概率就是零多项式 $z_H(X)$ 这句话如何理解？
...
其中乘法群的生成元为 $g=2$。由于 $13-1=3 \times 2 \times 2$，所以存在一个阶数为 $4$ 的乘法子群，其生成元为 $\omega=5$：
...
> ❓它的共轭 $\overline{\omega_i}=\omega^{-k}$ 改为 $\overline{\omega^i}=\omega^{\frac{N}{2}+i}$ ？
