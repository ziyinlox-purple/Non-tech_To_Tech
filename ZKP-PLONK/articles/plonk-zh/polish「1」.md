...
结合上面的电路比较图和矩阵 $W$ 一起来看，如果让 Prover 直接把电路赋值填入矩阵 $W$ 中，一个「诚实的」Prover 会在 $(w_a,1)$ （第一行第一个）和 $(w_c,2)$（第二行第三个） 两个位置填上相同的值；而一个「恶意的」Prover 完全可以填上不同的值。如果恶意的 Prover 在 $(w_b,1)$ 和 $(w_c,3)$ 也填入不同的值，那么实际上 Prover 证明的是上图右边的电路，而非是和 Verifier 共识过的电路(左边)。
...
然后把应该相等的两个位置互换，比如上图中要求 $(w_a,1)=(w_c,2)$ 和 $(w_b,1)=(w_c,3)$ 。于是我们得到了下面交换后的位置向量：
...
> 在描述中， $(w_i,j)$ 表示元素 $w_i$ 被交换到了新的位置 $j$。
...
为了保证 $W$ 矩阵中的第一行的 $w_c$ 也必须为 $99$（确保输出 $out$ 在所有相关位置都正确反映），这就需要在 $\sigma$ 向量中添加额外的一条拷贝约束：确保 $out$ 变量的位置 $(w_c,1)$ 与 第四行的输出 $(w_c,4)$ 交换对调：
