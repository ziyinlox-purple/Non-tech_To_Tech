...
第二步：用结果 $e$ 继续乘以 $c$ ，得到 $e \cdot c=f$；
...
不难发现，「中间列」向量 $\{r_i\}$ 向上挪一行与「最右列」 $\{q_i \cdot r_i \}$ 几乎一致，除了最后一个元素 $r_{n-1}=p$ 。该向量 $\vec{r}$ 的第一个元素用了常数 $1$ 作为计算初始值，「最右列」最后一个向量元素  $r_{n-1}$ 则记录了最终的计算结果。
...
> 📝 关于第 4 点的理解，为什么 $X$ 的取值范围要排除 $\omega^{-1}$ ? 因为 $\omega^{-1}$ 就是 $\omega^{n-1}$ ，对应的约束是 $q(\omega^{n-1}) \cdot r(\omega^{n-1}) = r(\omega \cdot \omega^{n-1})$ ，但 $q_{n-1} \cdot r_{n-1} = 0$ ，与 $r(\omega \cdot \omega^{-1}) = r_0 = 1$ 不相等，因此我们需要将 $x = \omega^{-1}$ 的这种情况排除。

> ❓ "$q_{n-1} = 0$ 的作用更像是一个特殊条件，以触发 $r_{n-1} = p$ 的特定条件" 这句话如何理解？
