...
- Verifier 使用双线性映射验证
$e(C_g - [26]_1, [1]_2) \overset{?}{=} e(\pi,[\chi-2]_2)$
...
但是如果 Verifier 只有 $g(X)$ 在 $\mathbb{G}_1$ 上的承诺 $[g(\chi)]_1$，而非是在 $\mathbb{G}_2$ 上的承诺 $[g(\chi)]_2$，那么 Verifer 就无法利用双线性配对操作来完成乘法检验。
...
但是，如果涉及的多项式较多，Prover 需要发送多个点值和求值证明，通信和计算成本较高。

我们可以进一步优化。为了减少打开的多项式数量，Prover 可以引入辅助多项式。此外，利用 KZG10 承诺的线性同态性，Verifier 可以在不增加通信成本的情况下验证复杂的乘法关系。

具体的操作如下：

首先，定义一个辅助多项式 $L(X)= \bar{f}\cdot g(X)-h(X)$，其中 $\bar{f} = f(\zeta) = c$，是 $f(X)$ 在点 $\zeta$ 的值。如果 $f(X) \cdot g(X) = h(X)$ 成立，则在点 $X = \zeta$，有：$L(\zeta)= \bar{f}\cdot g(\zeta)-h(\zeta)=0$

Prover 的操作：

1. 先打开 $f(X)$ 在点 $\zeta$ 的值：发送 $\bar{f} = f(\zeta)$ 和对应的求值证明 $\pi_{f(\zeta)}$ 给 Verifier。

2. Prover 再构造 $L(X)$ 的承诺： $[L(\chi)] _1 = \bar{f} \cdot [g(\chi) _1 - [h(\chi) _1]]$，但这个承诺不需要发给 Verifier。对于 Verifier 而言，它在收到 $\bar{f}$ 之后，就可以利用承诺的加法同态性，直接构造 $L(X)$ 的承诺： $[L(\chi)]_1 = \bar{f}\cdot [g(\chi)]_1 - [h(\chi)]_1$

3. 打开 $L(X)$ 在点 $\zeta$ 的值：Prover 证明 $L(\zeta) = 0$，即提供一个求值证明 $\pi_{L(\zeta)}$。

Verifier 的操作:

1. 验证 $f(\zeta)$ 的求值证明： $e([f(\chi)]_1, \bar{f}\cdot[1]_1, [1] _2)\overset{?}{=} e(\pi\_{f(\zeta)}, [\chi-\zeta]_2)$

2. 验证 $L(X)$ 的求值是否为零： $e([L(\chi)]_1, [1]_2)\overset{?}{=} e(\pi\_{L(\zeta)}, [\chi-\zeta]_2)$，如果验证通过，则说明 $f(X) \cdot g(X) = h(X)$。

这个优化过后的方案，Prover 原本需要打开三个多项式（$f(X)$, $g(X)$, $h(X)$），现在只需打开两个多项式（$f(X)$ 和 $L(X)$）。第一个 Opening 为 $\bar{f}$ (提供 $f(\zeta)$ 的具体值，以确保后续计算的准确性)，第二个 Opening 为 $0$ (证明辅助多项式 $L(X)$ 在 $\zeta$ 处为零，间接验证乘积约束)。我们仍然可以用上一节提供的聚合证明的方法，通过一个挑战数 $\nu$，Prover 可以聚合两个多项式承诺，然后仅需要发送一个求值证明。


我们下面尝试优化 $6$ 个多项式的约束关系的协议： $f_1(X)f_2(X) + h_1(X)h_2(X)h_3(X) + g(X) = 0$。
