...
插值需要用到一组插值多项式： $\{L_i(X)\}_{i\in[0,N-1]}$ 

其中 $L_i(w_i)=1$，

并且对于 $(j\neq i)$，$L_i(w_j)=0 (j\neq i)$，

这组插值多项式又被称为拉格朗日多项式（Lagrange Polynomials），看起来 $L_i(X)$ 像是一个选择器，这意味着：当 $X=w_i$ 时，只有 $L_i(X)$ 为 1，其他所有的 $L_j(X)(j\neq i)$ 都为 0。因此，拉格朗日多项式 $L_i(X)$ 在 $X=w_i$ 时「选择」了对应的系数 $a_i$，而忽略了其他系数。
...
举个更具体的例子，假设我们要构造多项式 $a(X)$，使得它在定义域 $H'={1,2,3}$ 上的取值为 $\vec{a}=(a_0,a_1,a_2)=(4,5,6)$
