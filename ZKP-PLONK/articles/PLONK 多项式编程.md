...
我们通过已有的三个插值点构造了「表2」 的拉格朗日插值多项式，有了它以后，**我们可以带入未知点 X 进行插值点的数值运算**，验证 $\sigma_a(X)⋅\sigma_b(X)\overset{?}=\sigma_c(X)$ 如果随机的这一次挑战检查 X 处计算的值没有问题，那么说明这个范围内的值都没有问题，Prover 没有作弊。
...
我们需要另一种多项式编码方案，利用 Lagrange Basis。如果我们要构造多项式 $a(X)$，使得它在定义域 $H=(w_0, w_1, \ldots w_{N-1})$ 上的取值向量为 $\vec{a}=(a_0,a_1, \cdots a_N-1)$，即
...
举个例子，假设我们要构造多项式 $a(X)$，使得它在定义域 $H'={1,2,3}$ 上的取值为 $\vec{a}=(a_0,a_1,a_2)=(4,5,6)$
